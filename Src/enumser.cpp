...
#pragma warning(suppress: 26446 26489 26490)
  nStatus = RegQueryValueEx(key, lpValueName, nullptr, &dwType, (BYTE*)(sValue.data()), &nBytes);
  if (nStatus != ERROR_SUCCESS)
  {
    SetLastError(nStatus);
    return false;
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryUsingSetupAPI(const GUID& guid, _In_ DWORD dwFlags, _Inout_ CPortAndNamesArray& ports)
{
  ...
  while (bMoreItems)
  {
    ...
    bMoreItems = SetupDiEnumDeviceInfo(hDevInfoSet, nIndex, &devInfo) != FALSE;
    if (bMoreItems)
    {
...
#pragma warning(suppress: 26429)
_Return_type_success_(return != false) bool CEnumerateSerial::IsNumeric(_In_z_ LPCSTR pszString, _In_ bool bIgnoreColon)
{
...
#pragma warning(suppress: 26429)
_Return_type_success_(return != false) bool CEnumerateSerial::IsNumeric(_In_z_ LPCWSTR pszString, _In_ bool bIgnoreColon)
{
